...
const processBlog = ({ blog, delimeter = '---', showFrontMatter = true }: Props): ReturnValue => {
  ...
  console.log(encodedDelimeter, 'encodedDelimeter');
  ...
  const frontMatterIndexes = [...blog.matchAll(new RegExp(encodedDelimeter, 'g'))].map(
    (match) => match.index
  );
  ...
  const processedBlog = blog.slice((frontMatterIndexes[1] || 0) + 3).trim();
